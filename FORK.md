...
| Substrate version the pin runs with | `0.0.27-gs.9` of the Giant Swarm line [giantswarm/substrate](https://github.com/giantswarm/substrate) — its release `v0.0.27-gs.9` (2026-09-13): upstream kagent-dev/substrate v0.0.26 (the `go/go.mod` replace, the ate-api contract) plus the line's carried patches, kagent-dev/substrate#33, giantswarm/substrate#25 (ateom re-asserts its capacity report every 10 s — giantswarm/giantswarm#37762) and giantswarm/substrate#30 (the gVisor worker's terminate collects a workload whose runsc containers are already gone, so a superseded template's crashed golden actor no longer pins its worker for good — giantswarm/giantswarm#37773) among them; the Substrate the platform's meta chart runs, whose worker `ghcr.io/giantswarm/substrate/ateom-gvisor:0.0.27-gs.9` the published chart stamps into `substrateWorkerPool.workerImage` (`SUBSTRATE_VERSION` / `SUBSTRATE_REPO` in the `Makefile`, read by `ci.yaml` through `make substrate-pin` and by `tag.yaml` at publish; `kubectl-ate` for the e2e bootstrap still from upstream's v0.0.26 release) |
...
| `fix(adk): allow a keyless OpenAI-compatible baseUrl (#2739)` | a `ModelConfig` with `provider: OpenAI`, an `openAI.baseUrl` and no `apiKeySecret`/`apiKeyPassthrough` reconciled `Accepted` while every Go ADK actor referencing it died at its golden boot (`failed to create LLM: OPENAI_API_KEY environment variable is not set`): the self-hosted OpenAI-compatible endpoints the platform's customers bring (vLLM, LiteLLM, corporate proxies) are keyless by design, and every one of them needed a placeholder Secret (giantswarm/kagent#57). The key is now resolved per provider — still required without a `baseUrl` (api.openai.com), and with a `baseUrl` and no key source the client sends no `Authorization` header; `NewOpenAICompatibleModel` (no callers) goes | kagent-dev/kagent#2739 — **merged** as `899977c5` (2026-09-14), carried as a cherry-pick until the next re-pin drops it. That re-pin (giantswarm/kagent-upstream#34) waits for the Substrate line: upstream `2d843e3` (kagent-dev/kagent#2802) moved kagent to Substrate v0.0.29, whose router addresses an actor by the `ate-target-actor` header that Substrate 0.0.27 (the line's `0.0.27-gs.9`) does not know |
