...
func TestReconciliationCollectionsCompileAndObserveRevision(t *testing.T) {
	...
	t.Run("crashed golden boot is started over and becomes Ready", func(t *testing.T) {
		...
		require.NoError(t, reconciler.reconcilePair(t.Context(), next.ResourceName()))
		require.Equal(t, []string{next.DesiredActorTemplate.GetMetadata().GetName()}, templates.deleted)
		waitFor(t, readyReason("ActorTemplatePending"))

		templates.template.Status = &ateapipb.ActorTemplateStatus{GoldenSnapshotStatus: &ateapipb.GoldenSnapshotStatus{GoldenTag: &ateapipb.ObjectRef{Atespace: "ate-golden", Name: "golden-2"}}}
		require.NoError(t, reconciler.reconcilePair(t.Context(), next.ResourceName()))
		waitFor(t, func() bool {
			updates := collections.AgentTemplateStatuses.List()
			return len(updates) == 1 && len(updates[0].Status.Harnesses) == 1 &&
