...
func TestReconcileRemovesCatalogWhenDiscoveryDisabled(t *testing.T) {
	...
	if catalog.deleted != want || catalog.server != nil {
		t.Fatalf("opted-out catalog = deleted %q, stored %#v; want %q deleted and nothing stored",
			catalog.deleted, catalog.server, want)
	}
}
